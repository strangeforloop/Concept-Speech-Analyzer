...
export function ConceptList() {
  ...
  return (
    <main className="min-h-screen bg-accent-50">
      ...
      <h1 className="mb-6 text-3xl font-bold text-neutral-800">Concept Practice</h1>

      <section className="mb-6 rounded-xl bg-neutral-100 p-4">
        <div className="grid gap-3 md:grid-cols-3">
          <select
            value={categoryFilter}
            onChange={(e) => setCategoryFilter(e.target.value)}
            className="rounded-md border border-neutral-200 bg-white px-3 py-2 text-neutral-700"
          >
          ...
          <label className="flex items-center gap-2 rounded-md bg-white px-3 py-2 text-neutral-700">
            <input
              type="checkbox"
              checked={showOnlyNew}
      ...
      {isLoading ? (
        <div className="rounded-xl border border-neutral-200 bg-white p-8 text-center text-neutral-600 shadow-sm">Loading concepts...</div>
      ) : null}

      {error ? (
        <div className="rounded-xl border border-warning-300 bg-warning-50 p-8 text-center text-warning-400 shadow-sm">
